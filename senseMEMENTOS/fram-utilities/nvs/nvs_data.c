...
nvs_status nvs_data_restore(nvs_data_handle handle, void *data, int ctpl_magic_number, int whichBox)
{
    uint16_t crc;
    uint8_t *data1;
    uint8_t *data2;
    ...
    if (header->token == NVS_DATA_TOKEN) {
        ...
        /*
        else if (which == NVS_DATA_2 && ctpl_magic_number == 0 ) {
...
nvs_status nvs_data_commit(nvs_data_handle handle, void *data,int whichBox)
{
    uint8_t *data1;
    uint8_t *data2;
    uint16_t framState;
