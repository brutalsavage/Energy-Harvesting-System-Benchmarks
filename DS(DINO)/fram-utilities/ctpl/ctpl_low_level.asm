...
CTPL_MAGIC_NUMBER_VALID .set    0x0001
CTPL_MAGIC_NUMBER_INVALID .set  0x0000
...
ctpl_stackCopyOld   .usect ".TI.persistent",CTPL_STACK_SIZE,2
; RAM copy
    .if $defined(CTPL_RAM_SIZE)
ctpl_ramCopy        .usect ".TI.persistent",CTPL_RAM_SIZE,2
ctpl_ramCopyOld     .usect ".TI.persistent",CTPL_RAM_SIZE,2
    .endif

; Low level state variables
ctpl_mode           .usect ".TI.persistent",2,2
ctpl_state          .usect ".TI.persistent",2,2
ctpl_magic_number   .usect ".TI.persistent",2,2

whichBox 			.usect ".TI.persistent",2,2

ctpl_stackUsage     .usect ".TI.persistent",2,2
ctpl_stackUsageOld  .usect ".TI.persistent",2,2

; Global symbols
    .global __STACK_END
	.global __STACK_END_OLD
	.global ctpl_magic_number
	.global whichBox
...
L30:
	...
	movx.w #CTPL_MAGIC_NUMBER_INVALID,&ctpl_magic_number
    jmp     ctpl_wakeup                         ; Yes, jump to wakeup
ctpl_initReturn:
    movx.w  #CTPL_MODE_NONE,&ctpl_mode          ; Reset the mode to none
    movx.w  #CTPL_STATE_INVALID,&ctpl_state     ; Mark the state as invalid
   	movx.w  #CTPL_MAGIC_NUMBER_VALID,&ctpl_magic_number
   	movx.w #BOX2, &whichBox
...
ctpl_saveCpuStackEnterLpm:
    ;movx.w #CTPL_MAGIC_NUMBER_INVALID,&ctpl_magic_number
    cmp #BOX2, &whichBox
    jnz ctpl_saveCpuStackEnterLpm2
...
ctpl_setStateValid:
    ...
    movx.w  #BOX1,&whichBox
    ;mov ctpl_stackUsageOld,ctpl_stackUsage
	;mov ctpl_stackCopyOld,ctpl_stackCopy
	;movx.w #CTPL_MAGIC_NUMBER_VALID,&ctpl_magic_number
    ;configureDcoWakeup
...
ctpl_saveCpuStackEnterLpm2:
    ...
    subx.a  SP,R4                               ; Calculate stack usage
    movx.w  R4,&ctpl_stackUsageOld                 ; Save stack usage
    movx.a  #ctpl_stackCopyOld,R6                  ; dest ptr
    movx.a  SP,R5                               ; src ptr
    copyx   R5,R6,R4                            ; copy the stack
    .if $defined(CTPL_RAM_SIZE)
    movx.w  #CTPL_RAM_START,R5
...
ctpl_wakeup2:
    ...
    .endif
    movx.a  #__STACK_END,SP                     ; Reset stack pointer
    movx.w  &ctpl_stackUsageOld,R4                 ; loop counter
    subx.a  R4,SP                               ; Reset stack pointer
    movx.a  SP,R6                               ; dest ptr
    movx.a  #ctpl_stackCopyOld,R5                  ; src ptr
    copyx   R5,R6,R4                            ; copy the stack
    popm.a  #8,R11                              ; Restore R4-R11 from stack
    ;movx.w  #CTPL_STATE_INVALID,&ctpl_state     ; Mark the state as invalid
ctpl_return2:
